...
currentWindow.listen("screenshot-captured", (event) => {
  ...
  const rawImageData = new Uint8ClampedArray(event.payload); // RGBA 格式
  const width = 2560;  // 设置图像宽度（请替换为实际截图宽度）
  const height = 1600; // 设置图像高度（请替换为实际截图高度）
  
  // 使用 ImageData 将 rawImageData 渲染到 canvas 上

  canvas.width = width;
  canvas.height = height;
  const imageData = new ImageData(rawImageData, width, height);
  ctx_background.putImageData(imageData, 0, 0);
  console.log("截图绘制完成!");
});



currentWindow.listen("screenshot-captured-old", (event) => {
  console.log("js收到截图!");
  ...
  img.onload = () => {
    console.log(`图像大小 ${img.height} ${img.width}`);

    // overlay1_canvas.height = img.height;
    // overlay1_canvas.width = img.width;
    ...
    console.log(`dpr: ${dpr}`);
    // 设置 canvas 的实际尺寸
    // const { offsetWidth, offsetHeight } = overlay1_canvas_background;

    // overlay1_canvas_background.width = offsetWidth * dpr;
    // overlay1_canvas_background.height = offsetHeight * dpr;

    console.log(`当前inner窗口大小 ${window.innerHeight} ${window.innerWidth}`);
    console.log(`当前outer窗口大小 ${window.outerHeight} ${window.outerWidth}`);

    // 设置 canvas 的显示尺寸
    // overlay1_canvas_background.style.width = `${img.width}px`;
    // overlay1_canvas_background.style.height = `${img.height}px`;
    ...
    console.log("绘制截图!");
    let scale = Math.min(
      overlay1_canvas_background.width / (img.width/dpr),
      overlay1_canvas_background.height / (img.height/dpr)
    );
    const newWidth = img.width * scale / dpr;
    const newHeight = img.height * scale / dpr;

    overlay1_canvas_background.width = newWidth * dpr;
    overlay1_canvas_background.height = newHeight * dpr;

    overlay1_canvas.width = newWidth * dpr;
    overlay1_canvas.height = newHeight * dpr;

    // 设置 canvas 的显示尺寸
    overlay1_canvas_background.style.width = `${newWidth}px`;
    overlay1_canvas_background.style.height = `${newHeight}px`;

    overlay1_canvas.style.width = `${newWidth}px`;
    overlay1_canvas.style.height = `${newHeight}px`;

    ctx = overlay1_canvas.getContext("2d");
    ctx_background = overlay1_canvas_background.getContext("2d");

    // 缩放 canvas 上下文
    ctx_background.scale(dpr, dpr);
    ctx.scale(dpr, dpr);
    ...
    ctx_background.drawImage(img, 0, 0, img.width, img.height, 0, 0, newWidth, newHeight);
  };
});
...
window.addEventListener("DOMContentLoaded", () => {
  ...
  main_overlay.appendChild(selection);
  ...
  overlay1_canvas.addEventListener("mouseup", (e) => {
    ...
    if (width >= 10 && height >= 10) {
      ...
      selection.addEventListener(
        "mouseenter",
        react_for_cursor_moveIn_selection
      );
      selection.addEventListener(
        "mouseout",
        react_for_cursor_moveOut_selection
      );
...
function build_up_selection_options_area() {
  ...
  selection_options_button_apply.addEventListener(
    "click",
    react_for_selection_option_accept
  );

  selection_options_button_cancel.addEventListener(
    "click",
    react_for_selection_option_refus
  );
}
...
function judge_event_in_area(e) {
  var mouse_x = e.clientX;
  var mouse_y = e.clientY;
  var selection_top = parseInt(selection.style.top, 10);
  var selection_left = parseInt(selection.style.left, 10);

  var selection_width = parseInt(selection.style.width, 10);
  var selection_height = parseInt(selection.style.height, 10);

  if (mouse_x > selection_left && mouse_x < selection_left + selection_width) {
    if (mouse_y > selection_top && mouse_y < selection_top + selection_height) {
      return true;
    }
  }
  return false;
...
function react_for_selection_mousedown(e) {
  console.log(`点击进入哪里? ${judge_event_in_area(e)}`);
  if (judge_event_in_area(e)) {
...
function react_for_selection_mouseup(e) {
  if (judge_event_in_area(e)) {
    ...
    selection_options_button_apply.removeEventListener(
      "click",
      react_for_selection_option_accept
    );
    selection_options_button_cancel.removeEventListener(
      "click",
      react_for_selection_option_refus
    );
